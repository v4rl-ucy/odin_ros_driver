...
static std::atomic<bool> deviceDisconnected(
    false);                      // Device disconnection flag
static std::mutex device_mutex;  // Device operation mutex lock
static std::atomic<bool> g_connection_timeout(false);
static std::atomic<bool> g_map_transfer_in_progress(
    false);  // True while a map file transfer is running
...
double cal_fps(fpsHandle* handle, const char* name, bool print = false) {
  ...
  if (print) {
    ...
    RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                "%s FPS: %f (count: %d, elapsed: %f)", name, fps, handle->count,
                elapsed);
#else
    ROS_INFO("%s FPS: %f (count: %d, elapsed: %f)", name, fps, handle->count,
             elapsed);
#endif
  }
...
class RosNodeControlImpl : public RosNodeControlInterface {
 public:
  ...
  bool sendOdomBaseLinkTF() const override { return pub_odom_baselink_tf; }
  ...
  int getTfExtraPublishRate() const override { return tf_extra_publish_rate; }
...
RosNodeControlInterface* getRosNodeControl() { return &g_rosNodeControlImpl; }
...
static bool convert_calib_to_cam_in_ex(const std::string& calib_path,
...
// Stop TF extra publish thread
    ...
    if (odinDevice) {
      ...
      if (g_ros_object) {
        const std::filesystem::path out_path =
            g_ros_object->get_root_dir() / "image" / "cam_in_ex.txt";
        (void)convert_calib_to_cam_in_ex(calib_file_, out_path);

#ifdef ROS2
        RCLCPP_INFO(rclcpp::get_logger("device_cb"), "pose_index: %d",
                    g_ros_object->get_pose_index());
        RCLCPP_INFO(rclcpp::get_logger("device_cb"), "cloud_index: %d",
                    g_ros_object->get_cloud_index());
        RCLCPP_INFO(rclcpp::get_logger("device_cb"), "image_index: %d",
                    g_ros_object->get_image_index());
#else
...
// Note: Map transfer is now triggered directly in process_command_file() when
// save_map=1 is set, so this monitor no longer needs to detect state changes.
// It's kept for potential future use or for monitoring other parameters.
static void custom_parameter_monitor() {
  while (g_param_monitor_running && deviceConnected) {
    // Currently no active monitoring needed since map transfer is triggered
    // directly in process_command_file() when save_map=1 is set. This loop is
    // kept for potential future parameter monitoring needs.
  ...
  if (std::getline(file, line)) {
...
#ifdef ROS2
      RCLCPP_WARN(
          rclcpp::get_logger("command_processor"),
          "Invalid command format. Usage: set <parameter_name> <value>");
#else
    ...
    if (command == "set") {
      ...
      try {
        ...
        // Early reject: if a map transfer is already running, ignore new
        // save_map=1 requests to avoid spurious "Successfully set save_map = 1"
        // followed by background failure.
        if (param_name == "save_map" && value == 1 &&
            g_map_transfer_in_progress.load()) {
#ifdef ROS2
          RCLCPP_WARN(
              rclcpp::get_logger("command_processor"),
              "Map transfer already in progress, ignoring this save_map=1 "
              "request. Please wait for current transfer to complete.");
#else
          ROS_WARN(
              "Map transfer already in progress, ignoring this save_map=1 "
              "request. Please wait for current transfer to complete.");
        ...
        // Special handling for "save_map = 1": delegate the entire flow (set ->
        // poll
        // -> fetch) to the SDK's unified lidar_save_map() API. We deliberately
        // skip the generic lidar_set_custom_parameter() call here, because
        // lidar_save_map() already issues that set internally; doing it twice
        // could re-trigger map generation on the device side after it already
        // completed once.
        if (param_name == "save_map" && value == 1) {
#ifdef ROS2
...
#endif
          {
            ...
            std::strftime(map_save_time, sizeof(map_save_time), "%Y%m%d_%H%M%S",
                          &tm);

            std::string map_dir = g_mapping_result_dest_dir != ""
                                      ? g_mapping_result_dest_dir
                                      : map_root_dir_.string();
            std::string map_name =
                g_mapping_result_file_name != ""
                    ? g_mapping_result_file_name
                    : "map_" + std::string(map_save_time) + ".bin";

#ifdef ROS2
            RCLCPP_INFO(rclcpp::get_logger("command_processor"),
                        "Map save triggered, will transfer to [%s/%s]",
                        map_dir.c_str(), map_name.c_str());
#else
            ROS_INFO("Map save triggered, will transfer to [%s/%s]",
                     map_dir.c_str(), map_name.c_str());
#endif
            ...
            // Run save-map in a background thread so we don't block the command
            // processor. All the orchestration (trigger -> poll -> fetch) is
            // now handled inside the SDK's lidar_save_map() one-shot API, so
            // this side just blocks on it and reports the final outcome.
            std::thread([map_dir, map_name]() {
              // 0 = use SDK default generation timeout (120s).
              int ret = lidar_save_map(odinDevice, map_dir.c_str(),
                                       map_name.c_str(), 0);
              ...
              if (ret == 0) {
#ifdef ROS2
                RCLCPP_INFO(rclcpp::get_logger("map_transfer"),
                            "Map transfer completed: %s/%s", map_dir.c_str(),
                            map_name.c_str());
#else
                ROS_INFO("Map transfer completed: %s/%s", map_dir.c_str(),
                         map_name.c_str());
#endif
              } else if (ret == -2) {
#ifdef ROS2
                RCLCPP_WARN(rclcpp::get_logger("map_transfer"),
                            "Map transfer skipped: device is busy with another "
                            "transfer");
#else
                ROS_WARN(
                    "Map transfer skipped: device is busy with another "
...
#ifdef ROS2
                RCLCPP_WARN(rclcpp::get_logger("map_transfer"),
                            "Map transfer failed, error code: %d", ret);
#else
        ...
        // Generic path for all other parameters (and save_map=0 /
        // save_map=N>1).
        int result = lidar_set_custom_parameter(odinDevice, param_name.c_str(),
                                                &value, sizeof(int));
        if (result == 0) {
#ifdef ROS2
          RCLCPP_INFO(rclcpp::get_logger("command_processor"),
...
#ifdef ROS2
          RCLCPP_INFO(rclcpp::get_logger("command_processor"),
                      "Device is busy transferring file, please wait for "
                      "current transfer to complete");
#else
          ROS_INFO(
              "Device is busy transferring file, please wait for current "
              "transfer to complete");
#endif
        } else {
#ifdef ROS2
          RCLCPP_ERROR(rclcpp::get_logger("command_processor"),
                       "Failed to set %s = %d, error: %d", param_name.c_str(),
                       value, result);
#else
          ROS_ERROR("Failed to set %s = %d, error: %d", param_name.c_str(),
                    value, result);
#endif
        }
...
#ifdef ROS2
        RCLCPP_ERROR(rclcpp::get_logger("command_processor"),
                     "Invalid value: %s", value_str.c_str());
#else
...
bool isUsb3OrHigher(const std::string& vendorId, const std::string& productId) {
  std::string command =
      "lsusb -d " + vendorId + ":" + productId + " -v | grep 'bcdUSB'";
  ...
  if (result.empty()) {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("usb_check"),
                 "Failed to get USB version information");
#else
...
#ifdef ROS2
    RCLCPP_ERROR(rclcpp::get_logger("usb_check"),
                 "bcdUSB field not found in lsusb output");
#else
...
#ifdef ROS2
  RCLCPP_INFO(rclcpp::get_logger("usb_check"), "Detected USB version: %.1f",
              version);
#else
...
bool isUsbDevicePresent(const std::string& vendorId,
      ...
      if (vendorFile.is_open()) {
        ...
        if (std::getline(vendorFile, vendorContent)) {
          ...
          std::string productPath =
              "/sys/bus/usb/devices/" + line + "/idProduct";
  ...
  try {
    if (calib_path.empty()) {
      ...
      RCLCPP_WARN(rclcpp::get_logger("device_cb"),
                  "calib_file_ is empty, skip writing cam_in_ex.txt");
#else
    ...
    if (!ofs.is_open()) {
      ...
      RCLCPP_ERROR(rclcpp::get_logger("device_cb"),
                   "Failed to open cam_in_ex.txt for write: %s",
                   out_path.string().c_str());
#else
      ROS_ERROR("Failed to open cam_in_ex.txt for write: %s",
                out_path.string().c_str());
#endif
      return false;
    }
...
#ifdef ROS2
    RCLCPP_INFO(rclcpp::get_logger("device_cb"), "Wrote cam_in_ex.txt to: %s",
                out_path.string().c_str());
#else
...
#ifdef ROS2
    RCLCPP_ERROR(rclcpp::get_logger("device_cb"),
                 "Failed to convert calib.yaml: %s", e.what());
#else
...
static void imu_thread_routine() {
...
#ifdef ROS2
  RCLCPP_INFO(rclcpp::get_logger("imu_thread"),
              "IMU dedicated thread started (priority: %d)",
  ...
  while (g_imu_thread_running) {
    ...
    // Wait for IMU data
    g_imu_queue_cv.wait(
        lock, []() { return !g_imu_queue.empty() || !g_imu_thread_running; });
...
static void start_imu_thread() {
  if (!g_imu_thread_running) {
    // 先确保之前的线程已经结束，避免 terminate called without an active
    // exception
...
static void lidar_data_callback(const lidar_data_t* data, void* user_data) {
  ...
  switch (data->type) {
    ...
    case LIDAR_DT_RAW_DTOF:
      if (g_senddtof) {
        g_ros_object->publishIntensityCloud(
            (capture_Image_List_t*)&data->stream, 1);
      }
    ...
    case LIDAR_DT_SLAM_CLOUD:
      if (g_sendcloudslam) {
        g_ros_object->publishPC2XYZRGBA((capture_Image_List_t*)&data->stream,
                                        0);
      }
    ...
    case LIDAR_DT_SLAM_ODOMETRY:
      if (g_sendodom) {
        g_ros_object->publishOdometry((capture_Image_List_t*)&data->stream,
                                      OdometryType::STANDARD, g_show_path,
                                      g_show_camerapose);
      }
    ...
    case LIDAR_DT_DEV_STATUS:
      ...
      if (g_devstatus_log) {
        if (dev_status_csv_file) {
          ...
          for (int i = 0; i < 8; i++) {
            rc = std::fprintf(dev_status_csv_file, "%d,",
          ...
          rc = std::fprintf(
              dev_status_csv_file, "%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,",
      ...
      if (g_show_fps) {
        ...
        printf(
            "\n [dev_info] [rgb]: configured_odr: %.2f HZ, tx_odr: %.2f HZ, "
            "rx_odr: %.2f HZ \n",
            ((float)dev_info_data->rgb_sensor.configured_odr) / 1000,
            ((float)dev_info_data->rgb_sensor.tx_odr) / 1000,
            cal_fps(&rgb_rx_fps, "rgb_rx"));
        ...
        printf(
            "\n [dev_info] [dtof]: configured_odr: %.2f HZ, tx_odr: %.2f HZ, "
            "rx_odr: %.2f HZ \n",
            ((float)dev_info_data->dtof_sensor.configured_odr) / 1000,
            ((float)dev_info_data->dtof_sensor.tx_odr) / 1000,
            cal_fps(&dtof_rx_fps, "dtof_rx"));
        ...
        printf(
            "\n [dev_info] [imu]: configured_odr: %.2f HZ, tx_odr: %.2f HZ, "
            "rx_odr: %.2f HZ\n",
            ((float)dev_info_data->imu_sensor.configured_odr) / 1000,
            ((float)dev_info_data->imu_sensor.tx_odr) / 1000,
            cal_fps(&imu_rx_fps, "imu_rx"));
        ...
        printf(
            "\n [dev_info] [slam]: slam_odom_highfreq_tx_odr: %.2f HZ, rx_odr: "
            "%.2f HZ \n",
            ((float)dev_info_data->slam_odom_highfreq_tx_odr) / 1000,
            cal_fps(&slam_odom_highfreq_rx_fps, "slam_odom_highfreq_rx"));

#ifdef ROS2
        RCLCPP_INFO(rclcpp::get_logger("dev_info"),
                    "Total RAM (parent + children): %.2f MB", total_mb);
#else
    ...
    case LIDAR_DT_SLAM_ODOMETRY_HIGHFREQ: {
      if (g_sendodom) {
        g_ros_object->publishOdometry((capture_Image_List_t*)&data->stream,
                                      OdometryType::HIGHFREQ, false, false);
      }
    ...
    case LIDAR_DT_SLAM_ODOMETRY_TF: {
      if (g_custom_map_mode == 2) {
        g_ros_object->publishOdometry((capture_Image_List_t*)&data->stream,
                                      OdometryType::TRANSFORM, false, false);
    ...
    case LIDAR_DT_NTP: {
      ...
      if (data_len == sizeof(ptp_sync_data_t)) {
        ptp_sync_data_t* ptp_data =
            (ptp_sync_data_t*)data->stream.imageList[0].pAddr;
        {
          ...
          if (!g_ptp_delay_buf.empty()) {
            g_ptp_delay_smooth.store(
                delay_sum / static_cast<double>(g_ptp_delay_buf.size()),
                std::memory_order_relaxed);
          }
          if (!g_ptp_offset_buf.empty()) {
            g_ptp_offset_smooth.store(
                offset_sum / static_cast<double>(g_ptp_offset_buf.size()),
                std::memory_order_relaxed);
          }
        }
...
static void lidar_device_callback(const lidar_device_info_t* device,
                                  bool attach) {
  ...
  if (attach == true) {
#ifdef ROS2
    RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                "Hardware connected, starting software connection...");
#else
    ...
    //                     "Device connected to USB 2.0 port. This device
    //                     requires USB 3.0 or higher. Exiting program.");
    //     #else
    //         ROS_FATAL("Device connected to USB 2.0 port. This device requires
    //         USB 3.0 or higher. Exiting program.");
    //     #endif
    ...
    if (lidar_create_device(const_cast<lidar_device_info_t*>(device),
                            &odinDevice)) {
#ifdef ROS2
    ...
    if (ros_workspace) {
      ...
      if (pos != std::string::npos) {
        config_dir =
            workspace_path.substr(0, pos) + "/src/odin_ros_driver/config";
      } else {
        config_dir =
      ...
      config_dir = ament_index_cpp::get_package_share_directory(package_name) +
                   "/config";
    }
#else
    ...
    std::filesystem::path per_con_log_root_dir;
    {
      ...
      std::filesystem::path base_log_dir =
          log_root_dir_.empty() ? std::filesystem::path(config_dir)
                                : log_root_dir_;
      per_con_log_root_dir = base_log_dir / folder_name;

      std::error_code per_con_dir_err;
      std::filesystem::create_directories(per_con_log_root_dir,
                                          per_con_dir_err);
      if (per_con_dir_err) {
#ifdef ROS2
        RCLCPP_WARN(rclcpp::get_logger("device_cb"),
    ...
    if (lidar_get_version(odinDevice, &version)) {
#ifdef ROS2
      RCLCPP_ERROR(rclcpp::get_logger("device_cb"),
                   "Failed to get device firmware version, potential "
                   "incompatible, please upgrade device firmware and retry.");
#else
      ROS_ERROR(
          "Failed to get device firmware version, potential incompatible, "
          "please upgrade device firmware and retry.");
#endif
      system("pkill -f rviz");
      ...
      RCLCPP_INFO(
          ...
          "ros_driver_version:%s, recommended_firmware_version:%d.%d.%d",
          ros_driver_version, required_firmware_version_major,
          required_firmware_version_minor, required_firmware_version_patch);
      RCLCPP_INFO(rclcpp::get_logger(__func__), "get version success.");
      RCLCPP_INFO(rclcpp::get_logger(__func__), "kernel_version: V%d.%d.%d",
                  version.kernel_version.major, version.kernel_version.minor,
                  version.kernel_version.patch);
      RCLCPP_INFO(rclcpp::get_logger(__func__), "mcu_version: V%d.%d.%d",
                  version.mcu_version.major, version.mcu_version.minor,
                  version.mcu_version.patch);
      RCLCPP_INFO(rclcpp::get_logger(__func__), "soc_version: V%d.%d.%d",
                  version.soc_version.major, version.soc_version.minor,
                  version.soc_version.patch);
      RCLCPP_INFO(
          rclcpp::get_logger(__func__), "Daemon_proc_version: V%d.%d.%d",
          version.Daemon_proc_version.major, version.Daemon_proc_version.minor,
          version.Daemon_proc_version.patch);
      RCLCPP_INFO(rclcpp::get_logger(__func__), "slam_version: V%d.%d.%d",
                  version.slam_version.major, version.slam_version.minor,
                  version.slam_version.patch);
#else
      ROS_INFO(
          "ros_driver_version:%s, recommended_min_firmware_version:%d.%d.%d",
          ros_driver_version, required_firmware_version_major,
          required_firmware_version_minor, required_firmware_version_patch);
      ROS_INFO("get version success.");
      ROS_INFO("kernel_version: V%d.%d.%d", version.kernel_version.major,
               version.kernel_version.minor, version.kernel_version.patch);
      ROS_INFO("mcu_version: V%d.%d.%d", version.mcu_version.major,
               version.mcu_version.minor, version.mcu_version.patch);
      ROS_INFO("soc_version: V%d.%d.%d", version.soc_version.major,
               version.soc_version.minor, version.soc_version.patch);
      ROS_INFO(
          "Daemon_proc_version: V%d.%d.%d", version.Daemon_proc_version.major,
          version.Daemon_proc_version.minor, version.Daemon_proc_version.patch);
      ROS_INFO("slam_version: V%d.%d.%d", version.slam_version.major,
               version.slam_version.minor, version.slam_version.patch);
#endif
      ...
      if (soc_version_too_low) {
        ...
        RCLCPP_ERROR(rclcpp::get_logger(__func__),
                     "The soc version is too low, please upgrade the device "
                     "firmware to at least %d.%d.%d\n",
                     required_firmware_version_major,
                     required_firmware_version_minor,
                     required_firmware_version_patch);
#else
        ROS_ERROR(
            "The soc version is too low, please upgrade the device firmware to "
            "at least %d.%d.%d\n",
    ...
    if (g_save_log) {
      if (lidar_enable_encrypted_device_log(
              const_cast<lidar_device_info_t*>(device),
              per_con_log_root_dir.c_str())) {
#ifdef ROS2
...
#ifdef ROS2
      RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                  "Encrypted device log enabled at: %s",
                  per_con_log_root_dir.c_str());
#else
      ROS_INFO("Encrypted device log enabled at: %s",
               per_con_log_root_dir.c_str());
#endif
    } else {
#ifdef ROS2
      RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                  "Encrypted device log disabled via configuration");
#else
    ...
    switch (device->initial_state) {
...
#ifdef ROS2
        RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                    "Device state: not initialized, performing full setup");
#else
...
#ifdef ROS2
        RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                    "Device state: initialized, skip opening device");
#else
      ...
      case LIDAR_DEVICE_STREAMING:
        ...
        RCLCPP_WARN(
            rclcpp::get_logger("device_cb"),
            "Device state: streaming, this should not happen, exitting...");
#else
        ROS_WARN(
            "Device state: streaming, this should not happen, exitting...");
#endif
...
#ifdef ROS2
        RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                    "Device state: stream stopped, resume streaming");
#else
...
#ifdef ROS2
        RCLCPP_WARN(rclcpp::get_logger("device_cb"),
                    "Unknown device initial state: %d", device->initial_state);
#else
    ...
    // Primary calib storage: writable runtime dir (~/.ros/odin_ros_driver by
    // default), independent of the source/install tree so it works for both
    // ROS1 and ROS2.
    ...
    bool need_calib_file =
        get_calib_file || !std::filesystem::exists(calib_config);
    if (need_calib_file) {
      if (lidar_get_calib_file(odinDevice, calib_runtime_dir.c_str())) {
#ifdef ROS2
        RCLCPP_ERROR(rclcpp::get_logger("device_cb"),
                     "Failed to get calibration file");
#else
...
// Print the primary save path
      ...
      RCLCPP_INFO(
          rclcpp::get_logger("device_cb"),
          "========== [CALIB] SAVED -> %s ==========", calib_config.c_str());
#else
      ROS_INFO("========== [CALIB] SAVED -> %s ==========",
               calib_config.c_str());
#endif

#ifdef ROS2
      RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                  "Successfully retrieved calibration files");
#else
...
#ifdef ROS2
      RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                  "Skipping calibration retrieval for current device state");
#else
    ...
    if (std::filesystem::exists(calib_config)) {
      try {
        ...
        std::filesystem::path backup_path =
            std::filesystem::path(config_dir) / "calib.yaml";
        std::filesystem::copy_file(
            calib_config, backup_path,
...
#else
        ROS_INFO("========== [CALIB] BACKUP -> %s ==========",
                 backup_path.string().c_str());
#endif
      } catch (const std::exception& e) {
#ifdef ROS2
        RCLCPP_WARN(rclcpp::get_logger("device_cb"),
                    "Failed to save calibration backup to config dir: %s",
                    e.what());
#else
        ROS_WARN("Failed to save calibration backup to config dir: %s",
                 e.what());
#endif
      }
    }
    ...
    // SN is parsed from the first comment line of calib.yaml (e.g. "#
    // O1-P010100043"). No-op when recorddata=0 (logger not initialized).
    if (g_ros_object) {
      char fw_buf[48];
      char algo_buf[48];
      ...
      if (std::filesystem::exists(calib_config)) {
        ...
        if (calib_in.is_open() && std::getline(calib_in, first_line)) {
          // Strip leading '#' and any whitespace, take token until end-of-line
          // / whitespace.
          size_t pos = first_line.find_first_not_of("# \t");
          if (pos != std::string::npos) {
            size_t end = first_line.find_first_of(" \t\r\n", pos);
            sn = first_line.substr(
...
#ifdef ROS2
        RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                    "Point cloud renderer initialized");
#else
...
#ifdef ROS2
      RCLCPP_WARN(rclcpp::get_logger("device_cb"),
                  "Renderer config file not found: %s", calib_config.c_str());
#else
    ...
    if (g_dtof_fps == 100) {
      ...
      RCLCPP_WARN(rclcpp::get_logger("ros[host_sdk_sample]"),
                  "Invalid dtof_fps value: %d, using default 14.5fps",
                  g_dtof_fps);
#else
...
// Log the configured frame rate
...
#ifdef ROS2
      RCLCPP_WARN(rclcpp::get_logger("device_cb"),
                  "Some custom parameters failed to apply");
#else
...
#ifdef ROS2
    RCLCPP_INFO(rclcpp::get_logger("device_cb"), "Custom map mode: %d",
                g_custom_map_mode);
#else
    ...
    if (g_custom_map_mode == 1) {
...
#ifdef ROS2
        RCLCPP_INFO(rclcpp::get_logger("command_processor"),
                    "Successfully initialized %s = %d", "save_map",
                    save_map_init_value);
#else
        ROS_INFO("Successfully initialized %s = %d", "save_map",
                 save_map_init_value);
#endif
      } else {
#ifdef ROS2
        RCLCPP_ERROR(rclcpp::get_logger("command_processor"),
                     "Failed to initialize %s = %d, error: %d", "save_map",
                     save_map_init_value, result);
#else
        ROS_ERROR("Failed to initialize %s = %d, error: %d", "save_map",
                  save_map_init_value, result);
#endif
      }
      ...
      if (g_relocalization_map_abs_path != "" &&
          std::filesystem::exists(g_relocalization_map_abs_path)) {
        while (retryTime-- > 0) {
          result =
              (lidar_set_relocalization_map(
                   odinDevice, g_relocalization_map_abs_path.c_str()) == 0);
          if (result) {
            break;
          }
#ifdef ROS2
          RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                      "Relocalization map set retry...,%d", retryTime);
#else
...
#ifdef ROS2
        RCLCPP_ERROR(rclcpp::get_logger("device_cb"),
                     "Relocalization map path set fail");
#else
    ...
    if (g_send_image_mask == 1) {
      if (g_image_mask_abs_path != "" &&
          std::filesystem::exists(g_image_mask_abs_path)) {
        int ret =
            lidar_set_image_mask(odinDevice, g_image_mask_abs_path.c_str());
        if (ret == 0) {
#ifdef ROS2
          RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                      "Image mask set successfully: %s",
                      g_image_mask_abs_path.c_str());
#else
          ROS_INFO("Image mask set successfully: %s",
                   g_image_mask_abs_path.c_str());
#endif
        } else {
#ifdef ROS2
          RCLCPP_ERROR(rclcpp::get_logger("device_cb"),
                       "Failed to set image mask: %s, error: %d",
                       g_image_mask_abs_path.c_str(), ret);
#else
          ROS_ERROR("Failed to set image mask: %s, error: %d",
                    g_image_mask_abs_path.c_str(), ret);
#endif
        }
      } else {
#ifdef ROS2
        RCLCPP_WARN(rclcpp::get_logger("device_cb"),
                    "Image mask path not set or file not found: %s",
                    g_image_mask_abs_path.c_str());
#else
        ROS_WARN("Image mask path not set or file not found: %s",
                 g_image_mask_abs_path.c_str());
#endif
      }
    }
    ...
    if (g_reset_algo == 1) {
      int reset_value = 1;
      int ret = lidar_set_custom_parameter(odinDevice, "algo_reset",
                                           &reset_value, sizeof(int));
      if (ret == 0) {
#ifdef ROS2
        RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                    "Algo reset command sent successfully");
#else
...
#ifdef ROS2
        RCLCPP_ERROR(rclcpp::get_logger("device_cb"),
                     "Failed to send algo reset command, error: %d", ret);
#else
    ...
    std::string dev_status_csv_file_path_ =
        per_con_log_root_dir / "dev_status.csv";
    ...
    if (!dev_status_csv_file) {
#ifdef ROS2
      RCLCPP_ERROR(rclcpp::get_logger("init"),
                   "Failed to open dev_status CSV file");
#else
      ROS_ERROR("Failed to open dev_status CSV file");
#endif
    } else {
      const char* header =
          "uptime_seconds,package_temp,cpu_temp,center_temp,gpu_temp,npu_temp,"
          "dtof_tx_temp,dtof_rx_temp,"
          "cpu0,cpu1,cpu2,cpu3,cpu4,cpu5,cpu6,cpu7,ram_use(%),"
          "rgb_configured_odr,rgb_tx_odr,rgb_rx_odr,dtof_configured_odr,dtof_"
          "tx_odr,dtof_rx_odr,imu_configured_odr,imu_tx_odr,imu_rx_odr,"
          "slam_cloud_tx_odr,slam_cloud_rx_odr,slam_odom_tx_odr,slam_odom_rx_"
          "odr,slam_odom_highfreq_tx_odr,slam_odom_highfreq_rx_odr,"
          "host_ram_use(mb)\n";
...
// 根据配置激活或关闭流类型
...
#ifdef ROS2
    RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                "Command interface ready. Use: echo 'set save_map 1' > %s",
                g_command_file_path.c_str());
#else
    ROS_INFO("Command interface ready. Use: echo 'set save_map 1' > %s",
             g_command_file_path.c_str());
#endif
    ...
    if (std::filesystem::exists(calib_config)) {
      ...
      RCLCPP_INFO(
          rclcpp::get_logger("device_cb"),
          "========== [CALIB] READ <- %s ==========", calib_config.c_str());
#else
      ROS_INFO("========== [CALIB] READ <- %s ==========",
               calib_config.c_str());
#endif
...
#ifdef ROS2
      RCLCPP_WARN(rclcpp::get_logger("device_cb"),
                  "========== [CALIB] NOT FOUND <- %s (skip camera params "
                  "load) ==========",
                  calib_config.c_str());
#else
      ROS_WARN(
          "========== [CALIB] NOT FOUND <- %s (skip camera params load) "
          "==========",
          calib_config.c_str());
#endif
    }
...
#ifdef ROS2
    RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                "Software connection successful in %ld seconds",
                std::chrono::duration_cast<std::chrono::seconds>(
                    std::chrono::steady_clock::now() - software_connect_start)
                    .count());
    RCLCPP_INFO(rclcpp::get_logger("device_cb"),
...
int main(int argc, char* argv[]) {
  ...
  auto srv_set_ae = node->create_service<odin_ros_driver::srv::SetAe>(
      ...
      [](const std::shared_ptr<odin_ros_driver::srv::SetAe::Request> req,
        ...
        lidar_cam_mode_e mode =
            (req->mode == 1) ? LIDAR_CAM_MODE_MANUAL : LIDAR_CAM_MODE_AUTO;
        int rc =
            lidar_set_ae_param(odinDevice, mode, req->exposure_time, req->gain);
        res->rc = rc;
        res->success = (rc == 0);
      });
  ...
  auto srv_set_awb = node->create_service<odin_ros_driver::srv::SetAwb>(
      ...
      [](const std::shared_ptr<odin_ros_driver::srv::SetAwb::Request> req,
        ...
        lidar_cam_mode_e mode =
            (req->mode == 1) ? LIDAR_CAM_MODE_MANUAL : LIDAR_CAM_MODE_AUTO;
        int rc = lidar_set_awb_param(odinDevice, mode, req->rgain, req->bgain);
        res->rc = rc;
        res->success = (rc == 0);
      });
  ...
  // -----------------------------------------------------------------
  ros::ServiceServer srv_get_ae =
      nh.advertiseService<odin_ros_driver::GetAe::Request,
  ...
  ros::ServiceServer srv_get_awb =
      nh.advertiseService<odin_ros_driver::GetAwb::Request,
  ...
  ros::ServiceServer srv_set_ae =
      nh.advertiseService<odin_ros_driver::SetAe::Request,
          ...
          boost::function<bool(odin_ros_driver::SetAe::Request&,
              ...
              [](odin_ros_driver::SetAe::Request& req,
                ...
                lidar_cam_mode_e mode = (req.mode == 1) ? LIDAR_CAM_MODE_MANUAL
                                                        : LIDAR_CAM_MODE_AUTO;
                int rc = lidar_set_ae_param(odinDevice, mode, req.exposure_time,
                                            req.gain);
                res.rc = rc;
                res.success = (rc == 0);
                return true;
  ...
  ros::ServiceServer srv_set_awb =
      nh.advertiseService<odin_ros_driver::SetAwb::Request,
          ...
          boost::function<bool(odin_ros_driver::SetAwb::Request&,
              ...
              [](odin_ros_driver::SetAwb::Request& req,
                ...
                lidar_cam_mode_e mode = (req.mode == 1) ? LIDAR_CAM_MODE_MANUAL
                                                        : LIDAR_CAM_MODE_AUTO;
                int rc =
                    lidar_set_awb_param(odinDevice, mode, req.rgain, req.bgain);
                res.rc = rc;
                res.success = (rc == 0);
                return true;
              }));
  ...
  try {
...
#ifdef ROS2
    RCLCPP_INFO(rclcpp::get_logger("init"), "Command file path set to: %s",
                g_command_file_path.c_str());
#else
...
#ifdef ROS2
      RCLCPP_ERROR(node->get_logger(), "Failed to load config file: %s",
                   config_file.c_str());
#else
    ...
    g_cloud_raw_confidence_threshold =
        get_key_value("cloud_raw_confidence_threshold", 35);
    g_rosNodeControlImpl.setCloudRawConfidenceThreshold(
        g_cloud_raw_confidence_threshold);
    g_dtof_fps = get_key_value(
        "dtof_fps",
        145);  // Read DTOF frame rate from config (100=10fps, 145=14.5fps)
    ...
    g_relocalization_map_abs_path =
        get_key_str_value("relocalization_map_abs_path", "");
    g_mapping_result_dest_dir =
        get_key_str_value("mapping_result_dest_dir", "");
    g_mapping_result_file_name =
        get_key_str_value("mapping_result_file_name", "");
    ...
    if (ros_workspace) {
      ...
      if (pos != std::string::npos) {
        data_dir =
            workspace_path.substr(0, pos) + "/src/odin_ros_driver/recorddata";
        log_dir = workspace_path.substr(0, pos) + "/src/odin_ros_driver/log";
        map_dir = workspace_path.substr(0, pos) + "/src/odin_ros_driver/map";
      } else {
        data_dir = ament_index_cpp::get_package_share_directory(package_name) +
                   "/recorddata";
        log_dir =
            ament_index_cpp::get_package_share_directory(package_name) + "/log";
        map_dir =
            ament_index_cpp::get_package_share_directory(package_name) + "/map";
      }
    } else {
      data_dir = ament_index_cpp::get_package_share_directory(package_name) +
                 "/recorddata";
    ...
    localtime_r(&t, &tm);
#endif
    std::strftime(driver_start_time, sizeof(driver_start_time), "%Y%m%d_%H%M%S",
                  &tm);

    if (g_devstatus_log) {
      std::string folder_name =
          std::string("Driver_") + std::string(driver_start_time);
...
#ifdef ROS2
      RCLCPP_INFO(node->get_logger(),
                  "Enabling SDK IMU smooth sending at %d Hz",
                  g_imu_smooth_frequency);
#else
      ROS_INFO("Enabling SDK IMU smooth sending at %d Hz",
               g_imu_smooth_frequency);
#endif
    } else {
#ifdef ROS2
    ...
    while (!deviceConnected) {
      ...
      if (usbPresent) {
        if (!usbVersionChecked) {
          ...
          //                          "Device connected to USB 2.0 port. This
          //                          device " "requires USB 3.0 or higher.
          //                          Exiting program.Please " "use USB 3.0 and
          //                          restart the device.");
          // #else
          //             ROS_FATAL(
          //                 "Device connected to USB 2.0 port. This device
          //                 requires USB " "3.0 or higher. Exiting program
          //                 .Please use USB 3.0 and " "restart the device.");
          // #endif
  ...
  if (odinDevice) {
    ...
    // {
    //     #ifdef ROS2
    //         RCLCPP_INFO(rclcpp::get_logger("device_cb"), "lidar_stop_stream
    //         failed");
    //     #else
    //         ROS_INFO("lidar_stop_stream failed");
    //     #endif
...
#ifdef ROS2
      RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                  "lidar_unregister_stream_callback failed");
#else
